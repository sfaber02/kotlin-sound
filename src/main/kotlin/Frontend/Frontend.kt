...
class Frontend {
    private val defaultTerminalFactory = DefaultTerminalFactory()
    private val terminal: Terminal = defaultTerminalFactory.createTerminal()
    private var screen: TerminalScreen = TerminalScreen(terminal)
    var textGraphics: TextGraphics = terminal.newTextGraphics()
    private var gui: WindowBasedTextGUI = MultiWindowTextGUI(screen)
    private var mainWindow: Window = BasicWindow("main")
    private var contentPanel: Panel = Panel(GridLayout(2))
    private var gridLayout = contentPanel.layoutManager as GridLayout

    private var rows: Int = screen.terminalSize.rows
    private var cols: Int = screen.terminalSize.columns

    fun buildUI() = runBlocking {
        screen.startScreen()
        ...
        mainWindow.component = oscillatorPanel
